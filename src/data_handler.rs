...
impl DataHandler {
    ...
    // pub fn promote_to_elder(&mut self, id: NodePublicId, config: &Config, total_used_space: &Rc<Cell<u64>>, init_mode: Init) -> Result<Self> {
    //     let idata_handler = IDataHandler::new(id.clone(), config, init_mode)?;
    //     let mdata_handler = MDataHandler::new(id.clone(), config, total_used_space, init_mode)?;
    //     let adata_handler = ADataHandler::new(id.clone(), config, total_used_space, init_mode)?;
    //     self.idata_handler = Some(idata_handler);
    //     self.mdata_handler = Some(mdata_handler);
    //     self.adata_handler = Some(adata_handler);
    //     Ok(self)
    // }
    ...
    fn handle_request(
        ...
        match request {
            ...
            // ===== Mutable Data =====
            //
            PutMData(data) => self.handle_mdata_request(|mdata_handler| mdata_handler.handle_put_mdata_req(requester, &data, message_id)),
            GetMData(address) => self.handle_mdata_request(|mdata_handler| mdata_handler.handle_get_mdata_req(requester, address, message_id)),
            GetMDataValue { address, ref key } => self.handle_mdata_request(|mdata_handler| mdata_handler.handle_get_mdata_value_req(requester, address, key, message_id)),
            DeleteMData(address) => self.handle_mdata_request(|mdata_handler| mdata_handler.handle_delete_mdata_req(requester, address, message_id)),
            GetMDataShell(address) => self.handle_mdata_request(|mdata_handler| mdata_handler.handle_get_mdata_shell_req(requester, address, message_id)),
            GetMDataVersion(address) => self.handle_mdata_request(|mdata_handler| mdata_handler.handle_get_mdata_version_req(requester, address, message_id)),
            ListMDataEntries(address) => self.handle_mdata_request(|mdata_handler| mdata_handler
            ...
            ListMDataUserPermissions { address, user } => self.handle_mdata_request(|mdata_handler|
                mdata_handler
    ...
    fn handle_put_idata_req(
        ...
        if &src == data.name() {
            ...
            self
            .handle_idata_request(|idata_handler|
